...
def load_data(dataset_name):
    if dataset_name == "cube":
        ...
        X_valid = torch.tensor(X_valid, dtype=torch.float32)
        y_valid = torch.tensor(y_valid, dtype=torch.float32)
    elif dataset_name == "synthetic_raw":
        ...
        y_valid = np.load('/work/users/d/d/ddinh/aaco/input_data/Synthetic_raw/test_labels.npy')
        
        y_train = np.eye(np.max(y_train) + 1)[y_train.astype(int)]
        y_valid = np.eye(np.max(y_valid) + 1)[y_valid.astype(int)]
        
        X_train = torch.tensor(np.concatenate([X_train_digits, X_train_counter], axis=1), dtype=torch.float32)
        X_valid = torch.tensor(np.concatenate([X_test_digits, X_test_counter], axis=1), dtype=torch.float32)
        y_train = torch.tensor(y_train, dtype=torch.float32)
        y_valid = torch.tensor(y_valid, dtype=torch.float32)
...
def load_classifier(dataset_name, X_train, y_train, input_dim):
    if dataset_name == "cube":
        ...
        xgb_model = XGBClassifier()
        xgb_model.load_model('/work/users/d/d/ddinh/aaco/models/synthetic_raw.model')
        return xgb_model
    else:
...
def get_knn(X_train, X_query, masks, num_neighbors, instance_idx=0, exclude_instance=True, classifier=None, num_ts=12, num_modality=4):
    ...
    distribution_train, embeddings_train = classifier.predict(X_train_masked, verbose=0)
    distribution_query, embeddings_query = classifier.predict(X_query_masked, verbose=0)
    ...
    if exclude_instance:
        ...
        return idx_topk[idx_topk != instance_idx][:num_neighbors], distribution_train[idx_topk != instance_idx][:num_neighbors]
    else:
        idx_topk = torch.topk(dist_squared, num_neighbors, dim=0, largest=False)[1]
        distribution = distribution_train[idx_topk]
        return idx_topk, distribution
...
def aaco_rollout(X_train, y_train, X_valid, y_valid, classifier, mask_generator, initial_feature, config):
    ...
    num_features = 20  # Replace with your actual number of features
    ...
    # Load the weights
    embedding_model.load_weights('/work/users/d/d/ddinh/aaco/models/embedding_synthetic_raw.weights.h5')
    ...
    for i in range(num_instances):  # Loop through the specified number of instances
        ...
        for j in range(num_ts + 1):
            if j == 0:
                ...
                # print('current: ', mask_curr)
                idx_nn, distribution_logits = get_knn(X_train, X[[i]], mask_curr.T, num_nb, i, not_i, embedding_model)
                # print("next_fea_pred", F.softmax(torch.tensor(next_fea_pred)[0], dim=0).numpy().argmax())
                idx_nn = idx_nn.squeeze()

                distribution = F.softmax(torch.tensor(distribution_logits), dim=-1).detach().numpy()
                distribution = distribution * (1 - mask_curr)
                
                for ts in range(num_ts):
                    distribution[:, ts * num_modality: ts * num_modality + flag_ts] = 0
                # import pdb; pdb.set_trace()
                next_feature = np.argmax(distribution, axis=-1)
                # if (next_feature % num_modality) >= flag_ts:
                candidate_mask = np.repeat(np.copy(mask_curr), num_nb, axis=0)
                candidate_mask[:, next_feature] = 1
                
                x_nb = X_train[[idx_nn]]
                y_nb = y_train[[idx_nn]]
    
                current_loss = loss_function(torch.tensor(classifier.predict_proba(np.concatenate([x_nb * mask_curr, np.repeat(mask_curr, num_nb, axis=0)], axis=1))), y_nb).detach().numpy()
                candidate_loss = loss_function(torch.tensor(classifier.predict_proba(np.concatenate([x_nb * candidate_mask, candidate_mask], axis=1))), y_nb).detach().numpy() + acquisition_cost
                diff_loss = current_loss - candidate_loss
                
                candidate_max_reduce = np.argmin(candidate_loss) #np.argmax(diff_loss)
                candidate_l = candidate_loss[candidate_max_reduce]
                current_l = current_loss[candidate_max_reduce]
                
                flag_ts = next_feature[candidate_max_reduce] % num_ts
                # import pdb; pdb.set_trace()
                if np.any(candidate_l < current_l):
                    # next_candidate = np.argmin(candidate_loss)
                    next_feature_candidate = next_feature[candidate_max_reduce]
                    
                    print(f"*** Next feature: {next_feature_candidate}")
                    print("***", X[[i]] * mask_curr)
                    mask_curr[0, next_feature_candidate] = 1
                    action = torch.zeros(1, feature_count + 1)
                    action[0, next_feature_candidate] = 1
                    mask_rollout.append(np.copy(mask_curr))
                    X_rollout.append(X[[i]])
        ...
        file_name = f"{results_dir}dataset_{config['dataset']}_embedding.pt"
...
if __name__ == "__main__":
    ...
    config = load_config("config_synthetic_raw.yaml")
