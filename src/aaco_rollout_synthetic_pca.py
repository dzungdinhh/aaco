...
def load_data(dataset_name):
    if dataset_name == "cube":
        ...
        initial_feature = 0
    elif dataset_name == "synthetic_pca":
        ...
        y_valid = np.load('/work/users/d/d/ddinh/aaco/input_data/pca_digits/test_labels.npy')
...
def load_classifier(dataset_name, X_train, y_train, input_dim):
    if dataset_name == "cube":
        ...
        xgb_model = XGBClassifier()
        xgb_model.load_model('/work/users/d/d/ddinh/aaco/models/synthetic_pca.model')
        return xgb_model
    else:
...
def get_knn(X_train, X_query, masks, num_neighbors, instance_idx=0, exclude_instance=True):
    ...
    masks_broadcasted = masks.unsqueeze(0).expand(X_train_squared.shape[0], X_train_squared.shape[1], 1)

    dist_squared = (
        (X_train_squared * masks_broadcasted).sum(dim=2, keepdim=True)  # [N, 20, 1]
        - 2.0 * (X_train_X_query * masks_broadcasted).sum(dim=2, keepdim=True)  # [N, 20, 1]
        + (X_query_squared * masks_broadcasted).sum(dim=2, keepdim=True)  # [N, 20, 1]
    )
    
    dist_squared = dist_squared.squeeze(-1).sum(dim=-1)
...
def aaco_rollout(X_train, y_train, X_valid, y_valid, classifier, mask_generator, initial_feature, config):
    ...
    for i in range(num_instances):  # Loop through the specified number of instances
        ...
        for j in range(num_ts + 1):
            ...
            if j == 0:
                ...
                x_rep = X_train[idx_nn].repeat(n_masks_updated, 1, 1)
                ...
                x_input = x_rep * mask_rep[:,:,None]
                x_input = x_input.reshape(x_input.shape[0], -1)
                y_pred = classifier.predict_proba(torch.cat([x_input, mask_rep], -1))
    ...
    file_name = f"{results_dir}dataset_{config['dataset']}_xgb_{config['acquisition_cost']}.pt"
...
if __name__ == "__main__":
    ...
    config = load_config("config_synthetic_pca.yaml")
