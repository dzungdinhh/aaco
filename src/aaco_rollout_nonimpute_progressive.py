        ...
        for j in range(num_ts + 1):
            ...
            if j == 0:
                ...
                for k in range(num_modality):  
                    ...
                    # replace the generated mask with the previous time step mask
                    mask[:, (k * num_ts): (k * num_ts) + int(smallests_val)] = current_mask[:,:int(smallests_val)]
                    
                    # replace the generated mask with the current mask, this is for progressively acquire the features
                    if current_mask[:,int(smallests_val)].item() == 1:
                        mask[:, (k * num_ts) + int(smallests_val)] = 1
                ...
                mask_zero_y = torch.Tensor(np.sum(y_rep, axis=-1) == 0).bool()
                try: 
                    for m in range(num_modality):
                ...
                if mask_diff.sum().item() == 0:
                    ...
                    action_idx_single = int(action_idx[0])
                    
                    print(smallests, action_idx, action_idx_single)
                    ...
                    mask_rollout.append(mask_curr.clone().detach())
                    action = torch.zeros(1, feature_count + 1)
                    action[0, action_idx_single] = 1
                    action_rollout.append(action)
                    
                    # Update the current mask
                    mask_curr[:, action_idx_single] = 1
