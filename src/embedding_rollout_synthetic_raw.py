    ...
    for i in range(num_instances):  # Loop through the specified number of instances
        ...
        for j in range(num_ts + 1):
            if j == 0:
                ...
                distribution = F.softmax(torch.tensor(next_fea_pred)[0], dim=0).numpy()
                ...
                print(f"*** Next feature: {next_feature}")
        ...
        file_name = f"{results_dir}dataset_{config['dataset']}_mlp_embedding_rollout.pt"
