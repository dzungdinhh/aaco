...
def load_data(dataset_name):
    if dataset_name == "cube":
        ...
        train_x = np.transpose(train_x, (0,2,1)).reshape(train_x.shape[0], -1)
        val_x = np.transpose(val_x, (0,2,1)).reshape(val_x.shape[0], -1)
        test_x = np.transpose(test_x, (0,2,1)).reshape(test_x.shape[0], -1)
        ...
        X_valid = torch.tensor(test_x, dtype=torch.float32)
        y_valid = torch.tensor(test_y, dtype=torch.float32)
...
def load_classifier(dataset_name, X_train, y_train, input_dim):
    if dataset_name == "cube":
        ...
        # return tf.keras.models.load_model('/work/users/d/d/ddinh/aaco/models/mlp_interpolation.keras')
...
def compute_accumulated_loss(y_pred, y_true, loss_function):
    ...
    for ts in range(y_pred.size(1)):
        ...
        timestep_loss = loss_function(y_pred[:, ts], y_true[:, ts]).reshape(-1)
...
def aaco_rollout(X_train, y_train, X_valid, y_valid, classifier, mask_generator, initial_feature, config):
    ...
    for i in range(num_instances):  # Loop through the specified number of instances
        ...
        for j in range(num_ts + 1):
            ...
            if j == 0:
                ...
                for ts in range(num_ts):
                    ...
                    pred = classifier.predict(np.concatenate([x_input * mask_input, ts_rep], axis=-1)).reshape(-1, 1)
                    y_pred[:,ts,:] = pred
                ...
                for m in range(num_modality):
                    modality_cost = 1 if m in [0, 2, 3] else 0.1
        ...
        file_name = f"{results_dir}dataset_{config['dataset']}_mlp_logistic.pt"
