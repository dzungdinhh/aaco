...
def load_data(dataset_name):
    if dataset_name == "cube":
        ...
        X_train = torch.tensor(Xtrn, dtype=torch.float32)
        y_train = torch.tensor(Ytrn, dtype=torch.float32)
        X_valid = torch.tensor(Xval, dtype=torch.float32)
        y_valid = torch.tensor(Yval, dtype=torch.float32)
        print(X_train.shape, y_train.shape, X_valid.shape, y_valid.shape)
        initial_feature = 0
    
    feature_count = 8  # Total number of features in the dataset
...
def load_classifier(dataset_name, X_train, y_train, input_dim):
    if dataset_name == "cube":
        ...
        xgb_model = XGBClassifier()
        xgb_model.load_model('/work/users/d/d/ddinh/aaco/models/digits.model')
        return xgb_model
    else:
    ...
    for i in range(num_instances):  # Loop through the specified number of instances
        ...
        for j in range(num_ts + 1):
            ...
            if j == 0:
                ...
                mask_input = np.copy(mask_curr)
                mask_input = np.expand_dims(mask_input, -1)
                mask_input = np.concatenate([mask_input]*2, 2)
                mask_input = np.transpose(mask_input, (0,2,1)).reshape(-1, mask_input.shape[1] * mask_input.shape[2])
                mask_input = torch.Tensor(mask_input)
                
                idx_nn = get_knn(X_train, X[[i]], mask_input.T, nearest_neighbors, i, not_i).squeeze()
                ...
                mask_input = np.copy(mask_rep)
                mask_input = np.expand_dims(mask_input, -1)
                mask_input = np.concatenate([mask_input]*2, 2)
                mask_input = np.transpose(mask_input, (0,2,1)).reshape(-1, mask_input.shape[1] * mask_input.shape[2])
                mask_input = torch.Tensor(mask_input)
                y_pred = classifier.predict_proba(torch.cat([x_rep * mask_input, mask_input], -1))
...
if __name__ == "__main__":
    ...
    config = load_config("config_digits.yaml")
