...
def get_potential_features(x, y, classifier, prev_masks, acquisition_cost, d=20, num_masks=1500, topk=5, num_timestamps=10, num_modalities=2):
    ...
    for i in range(x.shape[0]):
        ...
        terminations.append(np.mean(termination))
    # terminations = np.array(terminations)
    # print("Percentage terminated: ", np.mean(terminations == 1))
...
for epoch in range(num_epochs):
    ...
    if val_loss < best_val_loss:
        ...
        model.save('/work/users/d/d/ddinh/aaco/models/siamese_synthetic_raw.h5')
        model.save('/work/users/d/d/ddinh/aaco/models/siamese_synthetic_raw.keras')
        model.save_weights('/work/users/d/d/ddinh/aaco/models/siamese_synthetic_raw.weights.h5')
